...
namespace Carbonfrost.UnitTests.Validation {
    ...
    public class AllValuesValidatorAdapterTests {
        ...
        public void Validate_should_apply_to_enumerable_as_an_object() {
            ...
            var validator = Validator.Redirect(required, ValidatorTarget.AllValues);

            var obj = new PDictionaryWithNull();
            var actual = validator.Validate(obj);
            Assert.Single(actual);
            Assert.Equal("Values are required", actual.Values.First().Message);
        }
    }
}
